...
// Better name suggestion: PendingImport
#[derive(Debug, Clone, Copy)]
pub struct UnresolvedImport<'vfs>(pub &'vfs Ast, pub ast::NodeIndex, pub ScopeId);
...
pub struct ImportResolver<'ctx, 'hir, 'vfs> {
    ...
    unresolved_imports: HashMap<ScopeId, Vec<UnresolvedImport<'vfs>>>,
...
impl<'ctx, 'hir, 'vfs> ImportResolver<'ctx, 'hir, 'vfs> {
    ...
    fn resolve_import(
        &mut self,
        &UnresolvedImport(ast, node_index, scope_id): &UnresolvedImport<'vfs>,
    ) -> Result<(), ScanError> {
    ...
    fn resolve_path_inner(
        ...
        let result = match kind {
            NodeKind::Id => {
                ...
                let Some(item) = self.ctx.scope_manager.resolve(name, scope_id) else {
                    return Err(ScanError::UnresolvedIdentifier {
    ...
    fn next(&self) -> Option<(ScopeId, Vec<UnresolvedImport<'vfs>>)> {
