...
pub struct VfsScopeScanner<'hir, 'ctx, 'vfs> {
    ...
    pub unresolved_imports: HashMap<ScopeId, Vec<PendingImport<'vfs>>>,

    pub unresolved_implementations: Vec<(&'hir Ast, ast::NodeIndex, ScopeId)>,
}
...
impl<'hir, 'ctx, 'vfs> VfsScopeScanner<'hir, 'ctx, 'vfs> {
    ...
    pub fn new(
        ...
        VfsScopeScanner {
            ...
            unresolved_implementations: Vec::new(),
        }
    }

    /// Scans the entire virtual file system, building scopes and collecting unresolved imports.
    pub fn scan_vfs(&mut self) -> ScanResult<HashMap<ScopeId, Vec<PendingImport<'vfs>>>> {
    ...
    fn add_import(&mut self, scope_id: ScopeId, import: PendingImport<'vfs>) {
    ...
    fn process_entry_file(
        ...
        for (parent_scope_id, import_index) in import_indices {
            let import = PendingImport(ast, import_index, parent_scope_id);
