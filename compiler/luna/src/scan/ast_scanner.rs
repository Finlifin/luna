...
impl<'hir, 'ctx> AstScopeScanner<'hir, 'ctx> {
    ...
    fn scan_module_def(
        ...
        let clauses_index = self.ast.get_children(item)[1];
        let clauses = self
            .ast
            .get_multi_child_slice(clauses_index)
            .ok_or_else(|| ScanError::InternalError("Invalid clauses slice".into()))?;

        for &clause in clauses {
            let _ = self.scan_clause(scope, hir_id, clause)?;
        }
    ...
    fn scan_function_def(
        ...
        let body_scope = self
            .ctx
            .scope_manager
            .add_scope(None, Some(parent_scope), true, hir_id)
            .map_err(|e| ScanError::ScopeCreationFailed {
                message: format!("Failed to create function body scope: {:?}", e),
                span: rustc_span::DUMMY_SP,
            })?;

        let clauses_index = self.ast.get_children(item)[4];
        let clauses = self
            .ast
            .get_multi_child_slice(clauses_index)
            .ok_or_else(|| ScanError::InternalError("Invalid clauses slice".into()))?;
        ...
        for &clause in clauses {
            let _ = self.scan_clause(body_scope, hir_id, clause)?;
        }

        let item = Item::new(name, hir_id, Some(body_scope)); // 函数是纯符号，没有子scope
        self.ctx
            .scope_manager
    ...
    fn scan_clause(
        ...
        let Some((kind, span, _)) = self.ast.get_node(clause) else {
            return Err(ScanError::InternalError("Invalid clause node index".into()));
        };

        let result = match kind {
            NodeKind::ClauseTypeDecl => {
                let id = self.ast.get_children(clause)[0];
                let name = self.hir.str_arena.intern_string(
                    self.ast
                        .source_content(id, &self.hir.source_map)
                        .ok_or_else(|| {
                            ScanError::InternalError("Failed to get source content".into())
                        })?,
                );
                let (clause_id, clause) = self.hir.put_clause(Clause::TypeDecl {
                    symbol: name,
                    owner: parent_hir_id,
                    self_id: 0,
                });
                self.ctx.scope_manager.add_clause(parent_scope, *clause);
                clause_id
            }
            ...
            NodeKind::ClauseDecl => {
                ...
                let ty_index = self.ast.get_children(clause)[1];
                let ty = self.hir.put(HirMapping::Unresolved(
                    self.file_id,
                    ty_index,
                    parent_hir_id,
                ));

                let (clause_id, clause) = self.hir.put_clause(Clause::Decl {
                    symbol: name,
                    ty,
                    default: None,
                    owner: parent_hir_id,
            ...
            NodeKind::ClauseOptionalDecl => {
                ...
                let default_index = self.ast.get_children(clause)[2];
                let default = Some(self.hir.put(HirMapping::Unresolved(
                    self.file_id,
                    default_index,
                    parent_hir_id,
                )));
            ...
            NodeKind::Requires | NodeKind::Ensures | NodeKind::Decreases => 0,
            _ => {
                return Err(ScanError::InvalidNodeType {
                    message: "Invalid clause kind".into(),
                    span,
                });
            }
        };
