...
pub trait AstVisitor {
    ...
    /// 访问三子节点
    fn visit_triple_children(
        &mut self,
        ast: &Ast,
        node_index: NodeIndex,
        kind: NodeKind,
        children: [NodeIndex; 3],
        source_map: &SourceMap,
    ) -> Self::Output;

    /// 访问四子节点
    fn visit_quadruple_children(
        &mut self,
        ast: &Ast,
        node_index: NodeIndex,
        kind: NodeKind,
    ...
    /// 访问单个子节点 + 多个子节点
    fn visit_single_with_multi_children(
        &mut self,
        ast: &Ast,
        node_index: NodeIndex,
        kind: NodeKind,
        single_child: NodeIndex,
        multi_children: &[NodeIndex],
        source_map: &SourceMap,
    ) -> Self::Output;

    /// 访问双子节点 + 多个子节点
    fn visit_double_with_multi_children(
        &mut self,
        ast: &Ast,
        node_index: NodeIndex,
        kind: NodeKind,
        double_children: [NodeIndex; 2],
        multi_children: &[NodeIndex],
        source_map: &SourceMap,
    ) -> Self::Output;

    /// 访问三子节点 + 多个子节点
    fn visit_triple_with_multi_children(
        &mut self,
        ast: &Ast,
        node_index: NodeIndex,
        kind: NodeKind,
        triple_children: [NodeIndex; 3],
        multi_children: &[NodeIndex],
        source_map: &SourceMap,
    ) -> Self::Output;
    ...
    fn visit_impl_trait_def(
        &mut self,
        ast: &Ast,
        node_index: NodeIndex,
        kind: NodeKind,
        trait_expr: NodeIndex,
        type_expr: NodeIndex,
        clauses: &[NodeIndex],
        body: NodeIndex,
        source_map: &SourceMap,
    ...
    fn default_visit_node(
        ...
        if let Some(kind) = ast.get_node_kind(node_index) {
            match kind.node_type() {
                NodeType::NoChild => self.visit_no_child(ast, node_index, kind, source_map),
                ...
                NodeType::SingleWithMultiChildren => {
                    ...
                    self.visit_single_with_multi_children(
                        ast,
                        node_index,
                        kind,
                        first_child,
                ...
                NodeType::DoubleWithMultiChildren => {
                    ...
                    let multi_children = ast.get_multi_child_slice(multi_children_node).unwrap();
                    self.visit_double_with_multi_children(
                        ast,
                        node_index,
                        kind,
                ...
                NodeType::TripleWithMultiChildren => {
                    ...
                    let multi_children = ast.get_multi_child_slice(multi_children_node).unwrap();
                    self.visit_triple_with_multi_children(
                        ast,
                        node_index,
                        kind,
                ...
                NodeType::NormalFormDefChildren => {
                    ...
                    self.visit_diamond_function_def(
                        ast,
                        node_index,
                        kind,
                        id,
                ...
                NodeType::ImplTraitDefChildren | NodeType::ExtendTraitDefChildren => {
                    ...
                    self.visit_impl_trait_def(
                        ast, node_index, kind, trait_expr, type_expr, clauses, body, source_map,
                    )
                }
...
impl AstVisitor for SExpressionVisitor {
    ...
    fn visit_no_child(
        ...
        let byte_start =
            (ast.get_span(node_index).unwrap().lo().0 - source_file.start_pos.0) as usize;
        let byte_end =
            (ast.get_span(node_index).unwrap().hi().0 - source_file.start_pos.0) as usize;
    ...
    fn visit_single_child(
        ...
        format!("({} {})", kind, self.visit_node(ast, child, source_map))
    }
...
pub fn dump_ast_to_s_expression(
    ast: &Ast,
    node_index: NodeIndex,
    source_map: &SourceMap,
) -> String {
